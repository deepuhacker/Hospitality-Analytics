...
GROUP BY dd.week_no )
SELECT wbo.week_no,
CONCAT(ROUND((COALESCE(wr.weekly_revenue, 0) * 100.0) / COALESCE((SELECT SUM(revenue_generated) FROM fact_bookings), 1),2),'%') AS Revenue_Percentage,
CONCAT(ROUND((wbo.weekly_bookings * 100.0) / (SELECT SUM(successful_bookings) FROM fact_aggregated_bookings), 2),'%') AS Bookings_Percentage,
